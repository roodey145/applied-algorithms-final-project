...
public class KArrayBinarySearch {
    ...
    private int addNodeClauseData(int[] sortedArray, int posIndex, Integer[] nodeClauseData, int shift) {
        ...
        if(nodeClauseData[0] != null) shift += 3;
        // Left Node Index
        tree[posIndex - 1] = nodeClauseData[0] == null ? -1 : shift; // nodeClauseData[0];
        
        // If there is a left node, then the next one should be shifted by extra 3
        if(nodeClauseData[2] != null) shift += 3;
    ...
    private void addNodeClause(int[] sortedArray, int posIndex, int leftNodeIndex, int centerNodeIndex, int rightNodeIndex){
    ...
    private int handleAddingTreeNode(int index, int low, int high) {
        if(low > high) return index - nodeClauseSize;
        ...
        if(centerNodeClauseData[0] != null){
            lowerBound = getLowerBound(low, high, centerNodeClauseData[1]);
            // upperBound = getLowerBound(centerNodeClauseData[0] + 1, high, centerNodeClauseData[1]);
            ...
            // Contains 3 info. Left-left node index, left-node index, right-left node index.
            Integer[] leftNodeClauseData = getNodeClauseData(lowerBound[0], lowerBound[1]);
            ...
            // int[] leftNodeLowerBound = getLowerBound(lowerBound[0], lowerBound[1], centerNodeClauseData[0]);
            // index = handleAddingTreeNode(index, leftNodeLowerBound[0], leftNodeLowerBound[1]);

            // index += nodeClauseSize;

            // // Handle the right side of the left node
            // int[] leftNodeUpperBound = getUpperBound(lowerBound[0], lowerBound[1], centerNodeClauseData[0]);
            // index = handleAddingTreeNode(index, leftNodeUpperBound[0], leftNodeUpperBound[1]);
        }
        ...
        //     index += nodeClauseSize;
        //     // Handle the left side of the left node
        //     int[] rightNodeLowerBound = getLowerBound(upperBound[0], upperBound[1], centerNodeClauseData[2]);
        //     index = handleAddingTreeNode(index, rightNodeLowerBound[0], rightNodeLowerBound[1]);

        //     index += nodeClauseSize;

        //     // Handle the right side of the left node
        //     int[] rightNodeUpperBound = getUpperBound(upperBound[0], upperBound[1], centerNodeClauseData[2]);
        //     index = handleAddingTreeNode(index, rightNodeUpperBound[0], rightNodeUpperBound[1]);
        // }
