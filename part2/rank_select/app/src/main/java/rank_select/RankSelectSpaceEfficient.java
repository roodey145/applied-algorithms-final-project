...
public class RankSelectSpaceEfficient implements RankSelectInterface {
    ...
    public RankSelectSpaceEfficient(int[] bits, int k) {
        ...
        for (int i = 0; i < n; i++) {
            if (bits[i] != 0) { // treat any non-zero as 1
                ...
                words[wordIndex] |= (1 << bitIndex); // put a one in the bitIndex position from right to left of the 32
                                                     // bit int
            }
        }
    ...
    public int rank(int index) {
        ...
        // To count ones after the ones from the last block before that one we need top
        // look
    ...
    public int select(int r) {
        ...
        while (wordIndex < numWords) {
            ...
            if (onesSoFar + bitCount >= r) {
                ...
                break; // we may have gone beyond the desired rank, so we go to step 3 to get the exact
                       // index
            } else {
                onesSoFar += bitCount;
                wordIndex++;
         ...
         * Then the r-th 1 is the (10 - 7) = 3rd 1 inside this word. So we would need
         * the third 1 inside the word
         */

        for (int bit = 0; bit < 32; bit++) {
            if (((word >>> bit) & 1) != 0) { // checks if the inspected bit is not equal to 0 (shift it to the end and
                                             // compare with 1)
                remaining--;
                if (remaining == 0) {
    ...
    // Helper Methods
    public static int mask(int bitIndex) {
        // Assuming the bitIndex will always be between 0-31

        int mask;
        if (bitIndex >= 31) { // Changed from == to >= since we made an assumtion
            mask = -1; // all 32 bits set, all 1s, (32 ones)
        } else { // Handles only
            mask = (1 << (bitIndex + 1)) - 1;
            // 1 << (3+1) = 1 << 4 = 00010000
    ...
    public int findBlockForRank(int r) {
        int left = 0;
    ...
    // /**
    // *
    // * @param startWordIndex The index of the first word/subBlock that will be
    // searched.
    ...
    // if (onesSoFar + bitCount >= r) {
    // // The r-th 1 is in this word
    // break; // we may have gone beyond the desired rank, so we go to step 3 to get
    // the exact index
    // } else {
    // onesSoFar += bitCount;
    // wordIndex++;
